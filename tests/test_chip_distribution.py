...
class TestAvgCost:
    """compute_avg_cost: 筹码加权平均成本."""

    def test_returns_none_for_empty(self):
        from app.services.chip_distribution import compute_avg_cost
        assert compute_avg_cost([]) is None

    def test_avg_cost_equals_midpoint_for_uniform_distribution(self):
        """均匀分布时平均成本接近价格区间中点。"""
        from app.services.chip_distribution import compute_avg_cost, compute_chip_distribution
        bars = [_make_bar(0, high=1.10, low=1.00, volume=1000)]
        dist = compute_chip_distribution(bars, bin_count=20)
        avg = compute_avg_cost(dist)
        assert avg is not None
        assert abs(avg - 1.05) < 0.02

    def test_avg_cost_weights_heavy_bins(self):
        """权重集中在高价位时平均成本上移。"""
        from app.services.chip_distribution import compute_avg_cost, compute_chip_distribution
        bars = [
            _make_bar(0, high=1.20, low=1.10, volume=3000),  # 高价位重仓
            _make_bar(1, high=1.05, low=0.95, volume=1000),  # 低价位轻仓
        ]
        dist = compute_chip_distribution(bars, decay=1.0, bin_count=40)
        avg = compute_avg_cost(dist)
        assert avg is not None
