...
def update_fund_nav(fund_code):
    try:
        ...
        if response.status_code == 200:
            ...
            # 方式4: 查找特定的价格标签 - 不限制颜色，因为颜色仅表示涨跌幅
            if not latest_nav_elem:
            ...
            if latest_nav_elem:
                ...
                if not nav_match:
                    ...
                    # 如果QDII特殊处理失败或不是QDII基金，尝试直接提取所有数字序列
                    if not nav_match:
                        # 优先匹配3-4位小数的数值
                ...
                if not date_match:
                    ...
                    if date_matches:
                        ...
                        print(f"从页面中提取到最新日期: {latest_date}")
                        date_match = type('obj', (object,), {'group': lambda s, x=1: latest_date})
