    ...
    if request.method == 'POST':
        ...
        if not fund_id and not name:
            try:
                ...
                if fund_info:
                    flash(_('基金 %(code)s - %(name)s 已通过爬虫自动添加', code=code, name=fund_info.name), 'success')
                    return redirect(url_for('funds.manage_funds'))
...
def delete_fund(fund_id):
    ...
    try:
        ...
        db.session.commit()
        flash(_('基金删除成功'), 'success')
    except Exception as e:
        db.session.rollback()
        flash(_('删除基金失败: %(error)s', error=str(e)), 'error')
    return redirect(url_for('funds.manage_funds'))
...
def crawl_fund_nav():
    ...
    if request.method == 'POST':
        ...
        if is_batch:
            ...
            msg = _('批量更新完成：成功 %(success_count)s 个，失败 %(fail_count)s 个', success_count=success_count, fail_count=fail_count)
            if fail_funds:
                msg += _('\n失败：%(funds)s', funds=', '.join(fail_funds[:5]) + ('...' if len(fail_funds) > 5 else ''))
            flash(msg, 'success')
        else:
            fund_id = request.form.get('fund_id')
            fund = Fund.query.get(fund_id)
            if fund:
                if update_fund_nav(fund.code):
                    flash(_('基金 %(code)s - %(name)s 净值更新成功', code=fund.code, name=fund.name), 'success')
                else:
                    flash(_('基金 %(code)s - %(name)s 净值更新失败', code=fund.code, name=fund.name), 'error')
...
def get_fund_info():
    if not current_user.is_main_account:
        return jsonify({'success': False, 'message': _('只有管理员账户可以执行此操作')})

    fund_code = request.args.get('code')
    if not fund_code or not fund_code.isdigit() or len(fund_code) != 6:
        return jsonify({'success': False, 'message': _('基金代码必须是6位数字')})
    ...
    try:
        ...
        return jsonify({'success': False, 'message': _('无法获取该基金信息，请检查基金代码是否正确')})
    except Exception as e:
        return jsonify({'success': False, 'message': _('获取基金信息时发生错误: %(error)s', error=str(e))})
...
def get_fund_30_day_average():
    ...
    if not fund_id or not fund_id.isdigit():
        return jsonify({'success': False, 'message': _('基金ID必须是有效的数字')})

    try:
        fund = Fund.query.get(int(fund_id))
        if not fund:
            return jsonify({'success': False, 'message': _('基金不存在')})

        nav_records = FundNavHistory.get_latest_navs(int(fund_id), 30)
        if not nav_records:
            return jsonify({'success': False, 'message': _('暂无净值历史数据')})
        ...
        return jsonify({
            ...
            'message': _('基于最近%(count)s个交易日计算的平均净值', count=len(nav_records)),
        })
    except Exception as e:
        return jsonify({'success': False, 'message': _('获取前三十日平均净值时发生错误: %(error)s', error=str(e))})
...
def add_fund_from_recognition():
    ...
    if not fund_code:
        flash(_('基金代码不能为空'), 'error')
        return redirect(url_for('funds.manage_funds'))

    existing = Fund.query.filter_by(code=fund_code).first()
    if existing:
        flash(_('基金 %(code)s - %(name)s 已存在于系统中', code=fund_code, name=existing.name), 'info')
    else:
        try:
            from app.services.crawler import update_fund_nav
            update_fund_nav(fund_code)
            ...
            flash(_('添加基金时出错: %(error)s', error=str(e)), 'error')
